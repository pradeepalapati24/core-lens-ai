...
const difficultyConfig: { value: Difficulty; label: string; icon: any; color: string }[] = [
  { value: "beginner", label: "Beginner", icon: Zap, color: "text-primary" },
  { value: "intermediate", label: "Intermediate", icon: Signal, color: "text-accent" },
...
export default function PracticePage() {
  ...
  const steps = [
    ...
    <div key="domain">
      ...
      <p className="text-muted-foreground mb-6">Choose your engineering domain</p>
      <div className="flex gap-2 mb-6">
        {(["all", "it", "core"] as const).map((c) => (
          <button
            key={c}
            onClick={() => setCategoryFilter(c)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${categoryFilter === c ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-surface-hover"}`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {filteredDomains.map((d) => (
          <button
            ...
            className={`text-left p-4 rounded-xl border transition-all hover:border-primary/40 hover:shadow-md ${selectedDomain?.id === d.id ? "border-primary bg-primary/5" : "border-border bg-card"}`}
          >
            <div className="text-2xl mb-2">{d.icon}</div>
            <div className="font-semibold">{d.name}</div>
            <div className="text-sm text-muted-foreground mt-0.5">{d.description}</div>
            <div className="mt-2 text-xs text-muted-foreground">{d.topics.length} topics</div>
          </button>
        ))}
      </div>
    </div>,
    ...
    <div key="topic">
      ...
      <p className="text-muted-foreground mb-6">{selectedDomain?.name}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {selectedDomain?.topics.map((t) => (
          <button
            key={t.id}
            onClick={() => { setSelectedTopic(t); setStep(2); }}
            className={`text-left p-4 rounded-xl border transition-all hover:border-primary/40 ${selectedTopic?.id === t.id ? "border-primary bg-primary/5" : "border-border bg-card"}`}
          >
    ...
    <div key="subtopic">
      ...
      <p className="text-muted-foreground mb-6">{selectedDomain?.name} → {selectedTopic?.name}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {selectedTopic?.subtopics.map((s) => (
          <button
            key={s.id}
            onClick={() => { setSelectedSubtopic(s); setStep(3); }}
            className={`text-left p-4 rounded-xl border transition-all hover:border-primary/40 ${selectedSubtopic?.id === s.id ? "border-primary bg-primary/5" : "border-border bg-card"}`}
          >
    ...
    <div key="difficulty">
      ...
      <p className="text-muted-foreground mb-6">{selectedDomain?.name} → {selectedTopic?.name} → {selectedSubtopic?.name}</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {difficultyConfig.map((d) => (
          <button
            key={d.value}
            onClick={() => setSelectedDifficulty(d.value)}
            className={`p-6 rounded-xl border text-center transition-all hover:border-primary/40 ${selectedDifficulty === d.value ? "border-primary bg-primary/5" : "border-border bg-card"}`}
          >
        ...
        <div className="flex items-center gap-2 mb-8">
          {["Domain", "Topic", "Subtopic", "Difficulty"].map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${i <= step ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>
                {i + 1}
              </div>
